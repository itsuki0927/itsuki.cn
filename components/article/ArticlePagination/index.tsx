    ...
    <div className='w-1/2 border-r border-solid border-white-2 bg-white py-4 px-6'>
      ...
      {prevArticle ? (
        <Link href={`/article/${prevArticle.id}`}>
          <h3 className='cursor-pointer text-xs text-gray-1 transition-colors duration-500 hover:text-dark-2'>
            {prevArticle.title}
          </h3>
        </Link>
      ) : (
        <h3 className='text-xs text-gray-1'>无</h3>
      )}
    </div>
    ...
    <div className='w-1/2 bg-white py-4 px-6 text-right'>
      ...
      {nextArticle ? (
        <Link href={`/article/${nextArticle.id}`}>
          <h3 className='cursor-pointer text-right text-xs text-gray-1 transition-colors duration-500 hover:text-dark-2'>
            {nextArticle.title}
          </h3>
        </Link>
      ) : (
